...
-- Set a random wallpaper every few minutes
local wall_timeout = 600
local function random_wallpaper()
  beautiful.wallpaper = io.popen("sh -c \"find ~/pics/wals/* | shuf -n 1 | xargs echo -n\""):read("*all")
  set_wallpaper(s)
end
local wallpaper_timer = gears.timer({ timeout = wall_timeout })
wallpaper_timer:connect_signal("timeout", random_wallpaper)
wallpaper_timer:start()

local theme                                     = {}
theme.dir                                       = os.getenv("HOME") .. "/.config/awesome/themes/custom"
theme.widget_dir                                = os.getenv("HOME") .. "/.config/awesome/awesome-wm-widgets"
...
local mytextclock = wibox.widget.textclock("<span font='Roboto Bold 5'> </span>%a %b %d %H:%M ")
...
-- Separators
local first     = wibox.widget.textbox(markup.font("Roboto Bold 3", " "))
local spr       = wibox.widget.textbox(' ')
local small_spr = wibox.widget.textbox(markup.font("Roboto Bold 4", " "))
local bar_spr   = wibox.widget.textbox(markup.font("Roboto Bold 3", " ") .. markup.fontfg(theme.font, "#777777", "|") .. markup.font("Roboto Bold 5", " "))
...
function theme.at_screen_connect(s)
    ...
    s.mywibox:setup {
        ...
        { -- Right widgets
            ...
            arrow(theme.bg_normal, "#444466"),
            wibox.container.background(wibox.container.margin(mytextclock, dpi(4), dpi(8)), "#444466"),
            arrow("#444466", "alpha"),
            s.mylayoutbox,
        },
    }
